      ...
      <div className={`consignes ${rob.className}`}>
        <div id="rectangle" style={{ backgroundColor: getItem().color }}>
          <div className="title-container">
            <h1
              className={`title text-center text-[30px] md:text-[70px] lg:text-[100px] mt-[20px] md:my-[40px] font-bold ${bebas.className}`}
            >
              {getItem().title}
            </h1>
          </div>
          <div id="circle" className="hidden md:block" style={{ backgroundColor: getItem().color }}></div>
          <Image id="logo" alt="image" className="bottom-0 mb-5 md:bottom-auto" src={getItem().logo} width="150" height="150" />
        </div>
          ...
          {getItem().cards.map((card, key) => (
            <div className="flex-1 flex justify-center" key={key}>
              <div className="rounded-lg max-w-sm flex flex-col justify-between">
                ...
                <div className="p-6">
                  <h5 className="text-center text-2xl mb-2 font-bold" style={{ color: getItem().color, ...rob.style }}>
                    {card.text}
                  </h5>
                </div>
              </div>
        ...
        <div className="section-2 border-8 rounded-md mx-20 md:mx-52 pb-5" style={{ borderColor: getItem().color, color: getItem().color }}>
          <div className="col-sm-4 landing-section2-pillar">
            <div>
              ...
              <h1
                style={rob.style}
                className="text-center text-sans my-10 text-[30px] sm:text-[60px] font-bold underline decoration-[10px]"
              >
                INTERDITS
              </h1>
          ...
          <h1 style={rob.style} className="text-center my-5 text-[30px] sm:text-[60px] font-bold underline decoration-[10px]">
            INTERDITS
          </h1>
          <p className="text-center font-bold text-[15px] sm:text-[30px]">{getItem().forbiddenText}</p>
        </div>
          ...
          <div className="container mx-auto flex flex-col p-6">
            <div className="divide-y divide-gray-700">
              {getItem().recycleSteps.map((step, key) => (
                <div className="grid justify-center grid-cols-4 p-8 mx-auto space-y-8 lg:space-y-0" key={key}>
                  <div className="flex items-center justify-center lg:col-span-1 col-span-full rounded-full bg-red">
